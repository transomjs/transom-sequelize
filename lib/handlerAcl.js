...
module.exports = function HandlerAcl(options) {
    ...
    function applyInsertDefaults(model, req, data) {
        ...
        data.aclOwner = data.aclOwner ? data.aclOwner : owner;
        data.aclPublicPrivs = data.aclPublicPrivs ? data.aclPublicPrivs : acldefault.public

        if (!data.aclGroup && !data.aclGroupPrivs) {
            let group = null;
            let groupPriv = 0;
            if (acldefault.group) {
