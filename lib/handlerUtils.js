...
module.exports = function HandlerUtils(options) {
	...
	function processSelectOperator(model, select) {
        ...
        if (select) {
            ...
            selectAttribs.map((attrib) => {
                if (model.rawAttributes.hasOwnProperty(attrib)) {
                    result.attributes.push(attrib);
                } else {
                    throw createError(400, 'Invalid entry in the _select list: ' + attrib);
                }
            });
        }
        return result;
	...
	function buildQuery(query, req, entity) {
		...
		try {
            for (let key in currentAttribs) {
				...
				if (!isQueryable) {
					throw createError(400, `${model.name}.${key} is not a queryable attribute.`);
				}
                const attribWhere = [];
			...
			if (err.status || err.statusCode) {
				...
				throw createError(400, 'Unable to build query: ' + err.message);
			}
		}
			...
			if (currentOps._sort) {
				...
				for (let sort of sortFields) {
					if (!model.rawAttributes.hasOwnProperty(sort.replace("-", ""))) {
						throw createError(400, 'Invalid sort attribute: ' + sort);
                    }
                    if (sort.charAt(0) == '-') {                        
	...
	function getStrongTypeValue(val, attrMeta) {
        ...
        switch (datatype.key.toLowerCase()) {
			...
			case "dateonly":
				// Sample "2014-01-31" or "2014-01-31T12:30:58.123"
				if (val.length == 10 || val.length == 16 || val.length == 19 || val.length == 23) {
					if (val.length == 10) {
						retval = new Date(val + "T00:00:00");
					} else {
						retval = new Date(val);
					}
					...
					throw new Error("Invalid string length for date-only parsing");
				}
				break;
            case "date":
			case "time":
    ...
    function getStrongPkValue(id, model) {
        if (id == null) {
            debug(`Id was not provided in ${model.name} operation.`);
            throw createError(400, 'ID is required');
        }
        if (model.primaryKeyAttributes.length === 0) {
            debug(`Sequelize model ${model.name} does not include an attribute identified as the primary key.`);
            throw createError(500, 'Primary key not defined');
        }
        if (model.primaryKeyAttributes.length > 1) {
            debug(`Sequelize model ${model.name} has multiple attributes(${model.primaryKeyAttributes.join(', ')}) defined as primary key.`);
            throw createError(500, 'PK operations not currently supported on compound keys');
        }
        const pkMeta = model.rawAttributes[model.primaryKeyAttributes[0]];
        const pkId = getStrongTypeValue(id, pkMeta);
        return pkId;
