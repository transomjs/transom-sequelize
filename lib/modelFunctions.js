    ...
    function modelFindById(entity, req) {
        ...
        return new Promise(function (resolve, reject) {
            ...
            if (model.primaryKeyAttributes.length === 0) {
                ...
                return reject(new restifyErrors.InternalError('Primary key not defined'));
            } else if (model.primaryKeyAttributes.length === 1) {
                const pkMeta = model.rawAttributes[model.primaryKeyAttributes[0]];
                id = handlerUtils.getStrongTypeValue(id, pkMeta);
            } else {
                debug(`Sequelize model ${entity.modelName} has multiple attributes(${model.primaryKeyAttributes.join(', ')}) defined as primary key.`);
                return reject(new restifyErrors.InternalError('Find by PK not currently supported on compound keys'));
            }
...
*/
    ...
    function modelDeleteBatch(entity, req) {
        ...
        return new Promise(function (resolve, reject) {
            ...
            if (!deleteIdList) {
                return reject(new Error(`Request body must contain an "${model.primaryKeyAttribute}" field containing the array of PK values to delete.`));
            }
            ...
            const query = {
                where: {
                    [model.primaryKeyAttribute]: { [Op.in]: deleteIdList }
                  }
            };
            model.destroy(query).then(function (result) {
